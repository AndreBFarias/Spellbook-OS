...
ir() {
    ...
    if [ ! -d "$DEV_DIR" ]; then
        __err "Diretório '$DEV_DIR' não encontrado."
        return 1
    fi
    ...
    local destino=$(find "$DEV_DIR" -maxdepth 2 -type d -name ".git" -prune | sed 's/\/\.git//' | sort | \
        fzf --height=50% --layout=reverse --border=rounded \
            ...
            --preview 'echo -e "\033[38;2;189;147;249m$(basename {})\033[0m"; echo ""; git -C {} log --oneline --graph -8 2>/dev/null || echo "(sem histórico git)"; echo ""; ls -la --color=always {} 2>/dev/null | head -15')
