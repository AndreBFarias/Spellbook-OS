...
**Como aplicar**: validador auto-invoca skill `validação-visual` se diff toca padrões (`*.tsx,*.jsx,*.vue,*.svelte,*.html,*.css,*.scss,src/ui/**,*textual*,*widget*,templates/**`) ou se projeto declara tipo TUI/GUI/Web no BRIEF.

## 3. Acentuação em linhas periféricas é ponto cego

**Origem**: Luna AUD-03 FEN-11 (`feedback_accent_in_peripheral_lines.md`). Revisor pegou 2 violações que passaram na auto-revisão: `Pragmatico` na citação filosófica (§12 GUIDE.md) e `não-SPRINT` em docstring de teste.
...
- Citação filosófica final (GUIDE.md §12)
