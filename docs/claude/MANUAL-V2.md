...
4. **Skill `validação-visual`** — pipeline 3-tentativas (scrot → claude-in-chrome → playwright). Claude nunca mais diz "impossível" sem provar 3 tentativas com log literal.
...
3. Validador-sprint (subagent Opus) aplica 14 checks universais + skill `validação-visual` se diff toca UI → veredicto.
...
2. **Screenshot UI** — skill `validação-visual` auto-invocada.
...
ORIGEM (versionada no Spellbook-OS):
  ~/.config/zsh/docs/claude/
    ...
    skills/validação-visual/SKILL.md
...
CONSUMO (symlinks em ~/.claude/):
  ...
  ~/.claude/GUIDE.md      → ~/.config/zsh/AI.md (já existia)
```
...
Para confirmar que tudo está funcionando após a implantação, rode [`VALIDAÇÃO-V2.md`](./VALIDAÇÃO-V2.md) — checklist executável passo a passo.
