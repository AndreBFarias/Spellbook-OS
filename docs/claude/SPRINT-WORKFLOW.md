...
/sprint-ciclo "adicionar gauge VRAM na TUI"
  ...
  |   - Lê BRIEF + GUIDE.md + exploração do codebase
  ...
  |         - Skill validação-visual auto-invocada se diff toca UI
  ...
  |   - Lê BRIEF + GUIDE.md + spec + diff + proof-of-work
  |   - Aplica 14 checks universais
  |   - Confere skill validação-visual se UI
...
- [`CAPACIDADES-VISUAIS.md`](./CAPACIDADES-VISUAIS.md) — pipeline da skill validação-visual
