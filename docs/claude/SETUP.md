...
~/.claude/                        (consumidor pelo Claude Code)
  ...
  PLUGINS.md        -> symlink
  SETTINGS.md       -> symlink
  GUIDE.md         -> symlink    (para ~/.config/zsh/AI.md)
...
~/.config/zsh/                    (scripts e integração shell)
  ...
  scripts/bootstrap-rico-brief.py
  AI.md                           (GUIDE.md global - Protocolo Universal v4.0)
```
...
- `validação-visual` — pipeline 3-tentativas (scrot/import -> claude-in-chrome MCP -> playwright MCP) para capturar evidência visual quando sprint toca UI/TUI/Web. Critério de sucesso: PNG + sha256 + descrição multimodal. Fallback "impossível" só aceito após 3 tentativas documentadas.
...
Ciclo /sprint-ciclo:
   ...
   validador-sprint (subagent opus, contexto isolado)
      -> 14 checks + skill validação-visual se UI
