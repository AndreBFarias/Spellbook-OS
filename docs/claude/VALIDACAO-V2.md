...
for f in \
  ...
  ~/.claude/skills/validação-visual/SKILL.md \
  ~/.claude/PLUGINS.md \
...
```bash
cd ~/.config/zsh
grep -r -l -E "\b(função|validação|configuração|descrição|execução|atenção|operação)\b" \
...
rm -rf ~/.claude/skills/validação-visual
