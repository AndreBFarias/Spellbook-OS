...
- <meta-regras do GUIDE.md §9 aplicáveis>
...
- Validação visual (se UI): skill `validação-visual` + PNG + sha256
...
- **Imediatamente após gravar, valide a acentuação do spec.** Se o projeto tem `scripts/validar-acentuacao.py` (ou caminho equivalente), execute:
  ```bash
...
- **PT-BR direto.** Zero emojis. Acentuação correta obrigatória (á, é, í, ó, ú, â, ê, ô, ã, õ, à, ç). Antes de retornar, rode `validar-acentuacao.py` no spec gerado conforme passo 5. Palavras frequentemente erradas: `não`, `função`, `canônicos`, `exclusão`, `próprio`, `legítimo`, `útil`, `histórico`, `através`, `código`.
