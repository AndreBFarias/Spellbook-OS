...
- **Freeze por hang do display AMD (amdgpu DMCUB/DMUB) — Aurora 2.3**: em máquina híbrida (AMD iGPU dirige o display + NVIDIA dGPU faz compute, ex.: Nitro-5), o controlador de display da AMD pode travar — a **tela congela** (o cursor do mouse até mexe, mas o resto fica estático). **NÃO é VRAM/RAM/NVIDIA.** Sintoma no kernel, em loop: `amdgpu ... [drm] *ERROR* Error queueing DMUB command: status=2` + `DMCUB error` (`sudo dmesg | grep -i DMUB`). O watchdog `amdgpu-dmcub-watchdog.timer` (a cada 30s) detecta (>=5 erros em 60s) e recupera automaticamente em **2 passos** — o passo 2 é a parte não-óbvia: (1) `amdgpu_gpu_recover` (MODE2 reset, recarrega o firmware do DMUB); (2) `gnome-shell --replace` — o GPU reset **invalida os contextos GL/EGL** de todos os apps, então o device volta mas a tela só redesenha após recriar o compositor. Manual: `sudo sh -c 'cat /sys/kernel/debug/dri/0/amdgpu_gpu_recover'` e então `DISPLAY=:1 XAUTHORITY=/run/user/1000/gdm/Xauthority gnome-shell --replace &`. Logs: `journalctl -t aurora-amdgpu-watchdog -n 50`. Cooldown de 120s; recidiva (>4/h ou >=3 em 10min) gera `~/Desktop/AURORA-GPU-AVISO.md` e pausa o auto-recover (provável hardware/firmware — reinstale `linux-firmware`). O watchdog também detecta sessão Wayland (onde `--replace` não se aplica) e cobre assinaturas extras de hang (`device wedged`, `ring timeout`, `dc_dmub ERROR`). **Botão de pânico — quando o automático não basta (ex.: o "estado morto residual", em que o device volta mas a tela não redesenha): aperte `Ctrl+Alt+0`** (atalho via xbindkeys, que dispara mesmo com a tela travada). 1x = recupera (gpu_recover + compositor); **2x em ~15s = reinicia a sessão gráfica** (vai ao login; o trabalho já salvo permanece). Teste seguro: `sudo AURORA_WD_DRY_RUN=1 ~/.config/zsh/aurora/amdgpu-dmcub-watchdog` e `sudo /usr/local/sbin/aurora-gpu-revive --dry-run`.
...
- **Post-mortem automático após crash/REISUB**: `oom-postmortem.service` roda 1x por boot via `WantedBy=multi-user.target`. Detecta 3 sinais: **S1** kernel OOM killer (`journalctl -b -1 -k`), **S2** PSI thrashing no mem-snapshot, **S3** shutdown sujo no `wtmp` (inclui Magic SysRq REISUB e power loss). Se positivo, gera `~/Desktop/OOM-POSTMORTEM.md` com diagnóstico. Logs do próprio postmortem em `journalctl -t oom-postmortem`. Boot limpo: silencioso, e remove relatório antigo se existir.
